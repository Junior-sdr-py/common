...
def task_5_delete_the_last_customer(con) -> None:
   ...
   """
    ...
    with con.cursor() as cursor:
        cursor.execute("SELECT * FROM customers")
        cursor.execute("DELETE FROM customers  WHERE customerid={}".format(len(cursor.fetchall())))
...
def task_8_count_customers_by_city(cur):
    ...
    cur.execute("SELECT city, COUNT(city) FROM customers GROUP BY city ORDER BY 1 DESC")
    return cur.fetchall()
...
def task_13_list_products_from_sweden_suppliers(cur):
    ...
    Returns: 3 records
    """
    cur.execute("SELECT * FROM suppliers WHERE city='Sweden'")
    return cur.fetchall()
...
def task_14_list_products_with_supplier_information(cur):
    ...
    """
    cur.execute(
        "SELECT productid, productname, unit, price, country, city, suppliername FROM products LEFT JOIN suppliers ON "
...
def task_15_list_customers_with_any_order_or_not(cur):
    ...
    cur.execute(
        "SELECT customerName, contactName, country, orderid FROM customers FULL JOIN orders ON "
